...
for contour in contours:
    ...
    if len(approx) == 3:
        if not 'Triangle' in shapes:
            shapes['Triangle']=1
        else:
            shapes['Triangle']=shapes['Triangle']+1
  
    elif len(approx) == 4:
        if not 'Quadrilateral' in shapes:
            shapes['Quadrilateral']=1
        else:
            shapes['Quadrilateral']=shapes['Quadrilateral']+1
  
    elif len(approx) == 5:
        if not 'Pentagon' in shapes:
            shapes['Pentagon']=1
        else:
            shapes['Pentagon']=shapes['Pentagon']+1
  
    elif len(approx) == 6:
        if not 'Hexagon' in shapes:
            shapes['Hexagon']=1
        else:
            shapes['Hexagon']=shapes['Hexagon']+1
  
    else:
        if not 'Circle' in shapes:
            shapes['Circle']=1
        else:
